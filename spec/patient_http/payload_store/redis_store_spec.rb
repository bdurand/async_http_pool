...
RSpec.describe PatientHttp::PayloadStore::RedisStore do
  before(:all) do
  ...
  describe ".register" do
    it "is registered as :redis adapter" do
      described_class
      expect(PatientHttp::PayloadStore::Base.lookup(:redis)).to eq(described_class)
    end
  end
  ...
  describe "#store" do
    it "stores data as JSON in Redis" do
      ...
      stored = redis.get("patient_http:payloads:test-key")
    ...
    it "sets TTL when configured" do
      ...
      ttl = redis.ttl("patient_http:payloads:test-key")
      expect(ttl).to be > 0
      expect(ttl).to be <= 60
    end
