...
RSpec.describe "PatientHttp::PayloadStore::S3Store", :s3 do
  before(:all) do
    skip "S3/Minio not available for testing" unless S3Helper.available?
  end

  let(:described_class) { PatientHttp::PayloadStore::S3Store }
  ...
  describe ".register" do
    it "is registered as :s3 adapter" do
      described_class
      expect(PatientHttp::PayloadStore::Base.lookup(:s3)).to eq(described_class)
    end
  end
  ...
  describe "#initialize" do
    ...
    it "defaults key_prefix to patient_http/payloads/" do
      store = described_class.new(bucket: bucket)
      expect(store.key_prefix).to eq("patient_http/payloads/")
    end
  end
  ...
  describe "#store" do
    it "stores data as JSON in S3" do
      ...
      object = bucket.object("patient_http/payloads/test-key")
